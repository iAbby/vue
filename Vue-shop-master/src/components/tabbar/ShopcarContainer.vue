            ...
            <div class="mui-card" v-for="(item, i) in goodslist" :key="item.id">
                <div class="mui-card-content">
                    <div class="mui-card-content-inner">
                        ...
                        <mt-switch 
                            v-model="$store.getters.getGoodsSelected[item.goods_id]"
                            @change="selectedChanged(item.goods_id, $store.getters.getGoodsSelected[item.goods_id])"></mt-switch>
                        <img :src="item.img_url">
                        <div class="info">
                            <h1> {{ item.title }}</h1>
                            <p>
                              <span class="price">￥{{ item.sell_price }}</span>
                              <numbox :initcount="$store.getters.getGoodsCount[item.goods_id]" :goodsid="item.goods_id"></numbox>
                              <!-- 问题：如何从购物车中获取商品的数量呢 -->
...
export default {
    ...
    methods: {
        getGoodsList() {
            ...
            console.log(idArr)
            // 获取购物车商品列表
            this.$http.get("shopcar/" + idArr.join(",")).then((response) => {
